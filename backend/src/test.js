...
const dexConfig = config[config.dex];
const tokenConfig = config[config.token];

const web3 = new Web3(config.eth.RPC_URL);

var exchangeLists = {};
const loadExchangeStatsForDex = async (router) => {
    const routerContract = new web3.eth.Contract(routerABI, router);
    const factoryContract = new web3.eth.Contract(factoryABI, await routerContract.methods.factory().call());

    exchangeLists = {};
    for (let i = 0; i < await factoryContract.methods.allPairsLength().call(); i++) {
        try {
            ...
            console.log("Lists Info[i] :", token0Symbol, token1Symbol, reserves, tokenPrice)
        }
        catch (err) {
            console.log(err);
        }
...
loadExchangeStatsForDex(config.dex.UniSwap);
